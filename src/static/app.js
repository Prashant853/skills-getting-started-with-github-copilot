      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.length
          ? details.participants.map((email) => `
              <li>
                <span>${email}</span>
                <button class="participant-remove-button" type="button" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Remove ${email}">×</button>
              </li>
            `).join("")
          ...
          button.addEventListener("click", async () => {
            ...
            try {
              const removeResponse = await fetch(
                `/activities/${encodeURIComponent(activityToRemove)}/participants?email=${encodeURIComponent(emailToRemove)}`,
                {
                  method: "DELETE",
                }
              );
